...
//
// A verdict can be ABSENT for four different reasons, and an operator reading a
// held run needs to know which: the gate is off, the engine is not ruled on,
// the budget is spent, or the verdict simply has not landed yet. A mandatory
// gate holds the run through that last case, so "nothing here yet" is a state
// the panel has to render rather than hide.
//
...
export function AIPolicyPanel({
  ...
  if (attrs === null) {
    ...
    if (reason && !blocking) {
      // The gate is off, or this engine is not ruled on. Say so quietly.
      return (

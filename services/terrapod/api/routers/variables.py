...
def _file_delivery_guard(*, value_source: str, value: str | None, key: str, hcl: bool) -> None:
    ...
    effective key, value and hcl flag — so a PATCH that only renames the key
    (moving a defaulted file name) or only turns hcl on is caught too.
    """
    if value_source != "vault":
    ...
    if ref.get("file") is not None and hcl:
        raise HTTPException(
            status_code=422,
            detail="`file` delivery cannot be combined with hcl: the variable's value "
            "becomes the file's path, which is not an HCL expression. Turn hcl off.",
        )
...
async def update_workspace_var(
    ...
    try:
        ...
        _file_delivery_guard(
            ...
            key=eff_key,
            hcl=bool(attrs["hcl"]) if attrs.get("hcl") is not None else bool(var.hcl),
        )

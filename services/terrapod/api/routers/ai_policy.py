...
def _not_evaluated_reason(run: Run, ws: Workspace | None) -> str | None:
    ...
    A run with no row is ambiguous on its own — the gate may be off, or the
    verdict may simply not have landed yet — and "waiting" versus "never
    coming" is exactly the distinction an operator staring at a held run needs.
    """
    if run.plan_only:

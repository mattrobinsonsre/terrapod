...
# cloud/platform architect review. Near-exact clone of the cost-summary
# endpoints; renders on top of the deterministic security-scan panel. Advisory
# only (never gates a run).


def _architecture_critique_json(
    run_id,
    ...
    *,
    ...
    """JSON:API body for an architecture-critique row.

    The natural-language fields default to the stored (canonical-language)
    values; pass translated overrides (with ``translated=True`` + the canonical
    ``language``) to serve a reader-locale view (#963/#1036).
    """
    return {
        "data": {
            "id": f"architecture-critique-{critique.id}",
            "type": "architecture-critiques",
            "attributes": {
                "status": critique.status,
                "critique": critique_text if critique_text is not None else critique.critique,
                "findings": findings if findings is not None else critique.findings,
                "risk-level": critique.risk_level,
                "model": critique.model,
                "input-tokens": critique.input_tokens,
                "output-tokens": critique.output_tokens,
                "error-message": critique.error_message,
                # Canonical language the critique is stored in; `translated` is
                # true when the served prose was translated on view (#767).
                "language": language,
                "translated": translated,
                "created-at": _rfc3339(critique.created_at),
                "updated-at": _rfc3339(critique.updated_at),
            },
            "relationships": {
                "run": {"data": {"id": f"run-{run_id}", "type": "runs"}},
            },
...
@extensions_router.get("/runs/{run_id}/architecture-critique")
async def show_architecture_critique(
    run_id: str = Path(...),
    locale: str | None = Query(None),
    user: AuthenticatedUser = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
) -> JSONResponse:
    """AI architecture critique for a run (#963/#1036).

    Terrapod-native; the optional AI reasoning layer over the deterministic
    security scan, riding the plan-analysis AI switch. Renders in the UI on top
    of the security-scan panel. Advisory only — never gates a run, never
    restates the deterministic scan verdict.

    404 when no critique exists yet — the UI uses this as the "not generated"
    signal (vs a `pending` row, which means "in flight"). The prose is stored in
    the deployment's `ai_summary.summary_language`; when the caller passes a
    `?locale=` for a different real language, the critique + each finding's
    `title`/`detail` are translated on view (Redis-cached, best-effort) and
    `translated`/`language` reflect that.
    """
    run = await _get_run(run_id, db)
    await _require_run_ws_capability(run, cap.RUN_READ, user, db)

    critique = (
        await db.execute(select(ArchitectureCritique).where(ArchitectureCritique.run_id == run.id))
    ).scalar_one_or_none()
    if critique is None:
        raise HTTPException(status_code=404, detail="no architecture critique for this run")

    canonical_language = settings.ai_summary.summary_language
    if critique.status == "ready" and locale:
        from terrapod.services import summary_translation

        result = await summary_translation.translate_architecture_critique(
        ...
        if result is not None:
            return JSONResponse(
                content=_architecture_critique_json(
                    run.id,
                    critique,
    ...
    return JSONResponse(
        content=_architecture_critique_json(run.id, critique, language=canonical_language)
    )


@extensions_router.post("/runs/{run_id}/architecture-critique/regenerate")
async def regenerate_architecture_critique(
    run_id: str = Path(...),
    user: AuthenticatedUser = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
) -> JSONResponse:
    """Re-fire the AI architecture critique for a run (#963/#1036).

    Anyone with workspace `read` can regenerate; it mutates no infrastructure
    and is centrally cost-gated by `ai_summary.daily_token_budget`. Upserts the
    row to `pending`, enqueues the `ai_architecture_critique` trigger BYPASSING
    the per-run dedup (an explicit operator click always goes through), returns
    202.

    409 when the run has no plan JSON to critique. 503 when AI is globally
    disabled.
    """
    from terrapod.services.scheduler import enqueue_trigger
    ...
    if not run.has_json_output:
        raise HTTPException(status_code=409, detail="run has no plan JSON to critique")
    ...
    stmt = pg_insert(ArchitectureCritique).values(**pending_values)
    stmt = stmt.on_conflict_do_update(
        index_elements=["run_id"],
        set_={
            "status": stmt.excluded.status,
    ...
    # No dedup_key → bypass the per-run dedup; an explicit click always fires.
    await enqueue_trigger("ai_architecture_critique", {"run_id": str(run.id)})

    return JSONResponse(status_code=202, content=_architecture_critique_json(run.id, critique))


def _architecture_critique_message_attr(msg: ArchitectureCritiqueMessage) -> dict:
    """JSON:API attributes block for a single architecture-critique chat turn."""
    return {
        "role": msg.role,
        "content": msg.content,
        "model": msg.model,
...
async def _resolve_architecture_critique_for_chat(
    run_id: str, user: AuthenticatedUser, db: AsyncSession
) -> tuple[Run, ArchitectureCritique, Workspace]:
    """Shared header for both critique-chat endpoints — run exists, user has
    workspace `read`, a ready critique exists (404/409 otherwise)."""
    run = await _get_run(run_id, db)
    await _require_run_ws_capability(run, cap.RUN_READ, user, db)
    critique = (
        await db.execute(select(ArchitectureCritique).where(ArchitectureCritique.run_id == run.id))
    ...
    if critique is None:
        raise HTTPException(status_code=404, detail="no architecture critique for this run")
    if critique.status != "ready":
        raise HTTPException(
            status_code=409,
            detail=f"architecture critique is '{critique.status}', not 'ready' — cannot start chat",
        )
    workspace = (
        await db.execute(select(Workspace).where(Workspace.id == run.workspace_id))
    ).scalar_one_or_none()
    if workspace is None:
        raise HTTPException(status_code=404, detail="workspace not found")
    return run, critique, workspace


@extensions_router.get("/runs/{run_id}/architecture-critique/messages")
async def list_architecture_critique_messages(
    ...
    """Full transcript of the AI architecture-critique chat thread (#963/#1036).

    Chronological order; empty list when no follow-ups posted yet. The initial
    critique/findings live on the parent ``ArchitectureCritique`` row — this
    returns ONLY the conversational follow-ups. With ``?locale=`` set to a
    different real language, each message is translated on view (best-effort;
    ``translated`` per row).
    """
    _run, critique, _ws = await _resolve_architecture_critique_for_chat(run_id, user, db)
    rows = (
        (
            await db.execute(
    ...
    data = []
    for row, translated_content in zip(rows, translations, strict=True):
        attrs = _architecture_critique_message_attr(row)
        attrs["translated"] = translated_content is not None
        if translated_content is not None:
            attrs["content"] = translated_content
        data.append(
            {
                "id": f"architecture-critique-message-{row.id}",
...
@extensions_router.post("/runs/{run_id}/architecture-critique/messages")
async def post_architecture_critique_message(
    ...
    """Post an operator follow-up + get the synchronous assistant reply (#963/#1036).
    ...
    """
    from terrapod.services import summary_translation
    from terrapod.services.architecture_critic import post_architecture_followup
    from terrapod.services.summariser import (
        FollowupBudgetExhausted,
        FollowupCapReached,
        FollowupDisabled,
    ...
    run, critique, workspace = await _resolve_architecture_critique_for_chat(run_id, user, db)

    if locale:
        content = await summary_translation.normalize_to_system_language(content, locale)

    try:
        assistant_row = await post_architecture_followup(
            db=db,
    ...
    attrs = _architecture_critique_message_attr(assistant_row)
    attrs["translated"] = False
    if locale:
        reply_tr = await summary_translation.translate_message(
            message_id=str(assistant_row.id), content=assistant_row.content, reader_locale=locale
    ...
    return JSONResponse(
        ...
        content={
            "data": {
                "id": f"architecture-critique-message-{assistant_row.id}",
                "type": "architecture-critique-messages",
                "attributes": attrs,
            }
        },

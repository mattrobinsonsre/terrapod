    ...
    async with AsyncSession(engine, expire_on_commit=False) as session:
        async with session.begin():
            ...
            if existing_user:
                ...
                if generated:
                    logger.info("Generated password: %s", admin_password)  # nosemgrep: python-logger-credential-disclosure
...
async def _bootstrap_pool(session: AsyncSession, pool_name: str) -> None:
    ...
    if existing_token:
        logger.info("Join token already exists for pool '%s', skipping", pool_name)  # nosemgrep: python-logger-credential-disclosure
    else:
        ...
        session.add(token)
        logger.info("Created join token for pool '%s'", pool_name)  # nosemgrep: python-logger-credential-disclosure
        if token_generated:
            logger.info("Generated join token: %s", raw_token)  # nosemgrep: python-logger-credential-disclosure

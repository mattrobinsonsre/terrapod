...
async def blocked_by(
    ...
    """Which gate holds a run.

    One of `run-task`, `policy`, `security-scan`, `ai-policy`; None if nothing
    holds it.

    **The two #1837 boundaries answer here, NOT in `post_plan_hold`.** That
    function is about a run stopped *after* its plan and pairs each gate with a
    TFE post-plan status (#1704); a run held at `pre_plan` is `queued` and one
    held at `pre_apply` is `planned`, so neither has a post-plan status to
    report and neither belongs in that vocabulary.

    Answering here is what matters for safety: the commit-status dispatcher
    reads `blocked_by`, and a run held at `pre_apply` used to report no gate —
    so the PR check went green, a required check was satisfied, and the pull
    request looked mergeable while the run was held and would never apply.

    `unresolved_gate` lets a list endpoint answer for a whole page from one
    query instead of one per run.
    """

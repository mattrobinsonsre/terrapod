...
#: endpoints, and the explicit PR-comment unlock command. Each is a deliberate
#: human action, not a side effect of a run finishing.
DELIBERATE_UNLOCKS = [
...
def test_nothing_but_the_lock_endpoint_acquires_it():
    """The premise the fix rests on: only the lock endpoint sets the flag."""
    setters = [
    ...
    assert setters and all("api/routers/tfe_v2.py" in s for s in setters), (
        "Something other than the lock endpoint now sets `workspace.locked`. If a "

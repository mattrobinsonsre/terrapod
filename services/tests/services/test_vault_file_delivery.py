...
class _Var:
    ...
    #: The flag's pre-#1435 name. Nothing on this line sets it, but the check
    #: honours it so a caller handing over the old attribute cannot slip past.
    hcl: bool = False
    sensitive: bool = True
...
class TestClaimTimeRefusals:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("flag", ["structured", "hcl"])
    async def test_structured_with_a_file_fails_the_run(self, flag):
        v = _Var("F", _ref(file={}), category="terraform", **{flag: True})
        with _read(), pytest.raises(VaultSourceError) as e:
            await resolve_vault_delivery([v], _settings())
        assert str(e.value) == (
            "variable 'F' uses vault file delivery with structured (formerly hcl) "
            "enabled; its value is the file's path, which is not a typed expression"
        )

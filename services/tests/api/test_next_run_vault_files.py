...
- a clash, an oversized file or structured (hcl) errors the run; an unavailable Vault puts it
...
def _rv(key, value, *, category="env", value_source="vault", structured=False):
...
async def _claim(resolved, *, read=None, engine="terraform") -> _Claim:
    lid = uuid.uuid4()
    run = MagicMock()
    run.id = uuid.uuid4()
...
class TestDelivery:
    ...
    async def test_a_pulumi_run_gets_the_same_files_and_paths(self):
        """The variable's value is the file's path whatever the engine: a Pulumi
        run is delivered exactly what a Terraform run is, and its program reads
        the path from the env var as any other tool would."""

        def resolved():
            # Fresh per claim: next_run rewrites each value to its path in place,
            # as it does to the per-request list in production.
            return [
                _rv("GOOGLE_APPLICATION_CREDENTIALS", _ref(file={"name": "gcp/adc.json"})),
                _rv("AWS_SHARED_CREDENTIALS_FILE", _ref(file={"name": "~/.aws/credentials"})),
            ]

        tf = (await _claim(resolved())).attrs
        pulumi = (await _claim(resolved(), engine="pulumi")).attrs
        assert pulumi["engine"] == "pulumi"
        assert pulumi["pulumi-stack"] == "default/smoke/dev"
        for key in ("env-vars", "terraform-vars", "vault-files"):
            assert pulumi[key] == tf[key], key
        assert {v["key"]: v["value"] for v in pulumi["env-vars"]} == {
            "GOOGLE_APPLICATION_CREDENTIALS": "/var/run/terrapod/files/gcp/adc.json",
            "AWS_SHARED_CREDENTIALS_FILE": "/home/runner/.aws/credentials",
        }
        assert SECRET not in _without_vault_files(pulumi)

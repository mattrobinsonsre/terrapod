...
class TestBulkUpdateScanAndAISettings:
    ...
    `security-scan-enforcement` is an ordinary mapped field on this line. On
    `main` it has to be judged against the matched set, because a Pulumi run has
    nothing for the scanners to read (#1567) -- there is no engine strategy
    layer here, so every workspace is scannable.
    """
...
class TestBulkUpdateRemainingSettings1763:
    ...
    One of them cannot be judged from the payload alone: `vcs-workflow` needs
    each workspace's VCS connection and auto-apply state.
    """

...
test.describe('Responsive harness (phone viewport)', () => {
  ...
  test('architecture critic panel fits the phone viewport on the Security tab (#963)', async ({ page }) => {
    // The AI architecture critic (#963/#1036) renders on top of SecurityPanel in
    // the run's Security tab. Per #719 it must be mobile-friendly. The critique +
    // scan data path is unreachable in CI (no AI, no runner), so the two GETs are
    // stubbed to render the panel; here we guard the mobile surface: no horizontal
    // page scroll, findings (incl. long resource addresses) reflow without an
    // inner scroll trap, and the Regenerate control is a real ~44px tap target.
    const token = getStoredToken();
    const wsName = uniqueName('e2e-critic-mob');
    const wsId = await createWorkspace(token, wsName);
    const runId = await seedRun(token, wsId, true);
      ...
      route.fulfill({
        ...
        body: JSON.stringify({
          data: {
            ...
            attributes: {
              status: 'ready',
              critique: 'The proposed architecture is medium risk overall.',
              'risk-level': 'medium',
              findings: [
                {
                  severity: 'high',
                  category: 'reliability',
                  title: 'Single point of failure in the data tier',
                  detail: 'The primary database has no multi-AZ replica configured.',
                  address:
                    'module.platform.module.database.aws_db_instance.primary_with_a_very_long_address',
                },
              ],
              model: 'claude-e2e',
              'input-tokens': 100,
    ...
    // The critique panel renders at phone width.
    await expect(page.getByRole('heading', { name: 'AI architecture review' })).toBeVisible({
      timeout: 15_000,
    });
    // A long resource address reflows rather than forcing a horizontal page scroll.
    await expect(
      page.getByText('primary_with_a_very_long_address', { exact: false }),
    ).toBeVisible();
    await expectNoHorizontalPageScroll(page);

    // The Regenerate control is a real button with a proper tap target (~44px).
    const regen = page.getByRole('button', { name: /Regenerate AI architecture review/i });
    await expect(regen).toBeVisible();
    const box = await regen.boundingBox();
    expect(box, 'regenerate button has a box').not.toBeNull();
    expect(box!.height, 'regenerate is a real tap target, not clickable text').toBeGreaterThanOrEqual(28);
  });
});

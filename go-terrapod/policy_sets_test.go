...
// Scoping was settable through Create/Update and never read back, so a caller
// could scope a policy set and then not see the scoping it had applied — which
// makes it undetectable drift for anything managing a set as code.
//
// The shape is a role's, deliberately: the server matches both with the same
// code (policy_set_service._labels_match), and on this line a role binds ONE
// value per key, so a policy set does too.
func TestPolicySetReadsBackItsScoping(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		...
		_, _ = w.Write([]byte(`{"data":{"type":"policy-sets","id":"ps-1","attributes":{
			...
			"allow-labels":{"env":"prod","team":"sre"},
	...
	if got := ps.AllowLabels["env"]; got != "prod" {
		t.Errorf("allow-labels env = %q, want prod", got)
	}
	if got := ps.AllowLabels["team"]; got != "sre" {
		t.Errorf("allow-labels team = %q, want sre", got)
	}
	if got := ps.DenyLabels["tier"]; got != "scratch" {
		t.Errorf("deny-labels tier = %q, want scratch", got)
	}
}

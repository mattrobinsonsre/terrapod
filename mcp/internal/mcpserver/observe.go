...
func registerObserve(s *mcp.Server, c *terrapod.Client) {
	...
	mcp.AddTool(s, &mcp.Tool{
		...
		Description: "Get a run's AI policy gate verdict (#1766): the model's allow/deny ruling against the operator's " +
			...
			"is coming\" — the gate being off, the engine not being ruled on (Pulumi), or the verdict simply not " +
			"having landed. `enforcement_level` here is the workspace's CURRENT setting, which can differ from the " +

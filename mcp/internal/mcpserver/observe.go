...
func registerObserve(s *mcp.Server, c *terrapod.Client) {
	...
	mcp.AddTool(s, &mcp.Tool{
		...
		Description: "Report the sampled status of every configured HashiCorp Vault instance Terrapod reads variable values from (#1663) — the first thing to read when a Vault-sourced variable will not resolve. Per instance: `reachable` plus sys/health (`initialized`, `sealed`, `standby`, `version`); `login-ok` with the configured auth method (kubernetes, jwt, approle or token) and `ttl-seconds` of Terrapod's Vault token; `tls-trust` (instance-ca = verified against that instance's own CA only, global-bundle = the chart's caBundle, default = the system store, skip-verify); and `last-error`, the last failed resolution from any run ({class, message, at} — class VaultUnavailable means runs are waiting in queued, VaultDenied means a policy or login refusal, VaultNotFound a wrong path). Every probe field is NULL when unknown — not yet sampled, or not attempted (a sealed Vault is never logged in to) — which is NOT the same as false. The status is sampled once a minute on the server, never by this call, so `checked-at` says how fresh it is. `enabled` false means the Vault value source is off. Never returns a secret value. Read-only; requires platform admin or audit.",
		Annotations: readOnly,
	...
	type vaultRefCheckIn struct {
		...
		Reference     map[string]any `json:"reference,omitempty" jsonschema:"a Vault reference exactly as a vault-sourced variable's value holds it, e.g. {\"mount\":\"secret\",\"path\":\"apps/x\",\"field\":\"token\"} with optional vault, engine (kv2|dynamic), method, data and file; give this or variable_id"`

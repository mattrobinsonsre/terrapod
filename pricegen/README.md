...
- [x] GCP computed engine + `google_compute_instance` (Σ vCPU-core + GiB-RAM, formulaic catalog, zone→region); `google_compute_disk` (pd-standard/balanced/ssd, per-GiB), `google_compute_address` (static IP hourly), `google_storage_bucket` (Standard regional, band) — validated end-to-end. GCP adapter skips leading $0 tiers; region resolution is case-insensitive (a bucket's uppercase `location`)

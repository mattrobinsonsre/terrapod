...
The published sheet covers **every** region. Nothing on the generate or consumer
side is region-specific: an AWS row's region comes from its product `regionCode`,
an Azure row's from `armRegionName`, GCP direct rows carry every `serviceRegion`,
and the consumer resolves each resource's region per-resource (#871) — so a plan
spanning regions is priced correctly. Region is purely a **fetch** concern.

Because the AmazonEC2 offer is ≈458 MB *per region*, doing all regions on one
runner is disk-/bandwidth-bound, so the weekly CI job (`pricesheet.yml`) **fans
out** — the work is split into shards, each fetching only its slice at flat
RAM/disk and emitting a *partial* sheet, then a `merge` job combines them:

| Shard | Flags | Covers |
|---|---|---|
| `aws-<region>` (one per region, discovered live from the EC2 region index) | `--only aws --region <r>` | that region's regional AWS recipes |
| `aws-global` | `--only aws --global-only` | Route53 + CloudFront (global offers, generated once) |
| `azure` | `--only azure --all-regions` | all Azure regions (one paginated pass, filter dropped) |
| `gcp` | `--only gcp` | all GCP regions (its catalog is already all-region) |

`pricegen.shard` is the single source both `fetch_offers` and `publish` use to
decide which recipes + regions a shard covers, so a shard can never generate a
recipe whose offer it didn't fetch. `pricegen.merge` then concatenates the
partial sheets' products and aggregates the per-recipe drift manifests.

```sh
# one shard (what a CI matrix leg runs): fetch its slice, emit a partial
python3 -m pricegen.fetch_offers --only aws --region us-west-2
python3 -m pricegen.publish --only aws --region us-west-2 \
  --out partial-aws-us-west-2.prices.yaml.gz --manifest partial-aws-us-west-2.manifest.json

# merge every shard's partials into the published sheet
python3 -m pricegen.merge --sheets 'partial-*.prices.yaml.gz' \
  --manifests 'partial-*.manifest.json' --out prices.yaml.gz --manifest manifest.json
```
...
- [x] Azure adapter + `azurerm_linux_virtual_machine`, `azurerm_windows_virtual_machine` (Windows meter), `azurerm_public_ip`, `azurerm_kubernetes_cluster` (AKS Standard tier), `azurerm_storage_account` (replication-matched, volume-tiered), `azurerm_managed_disk` (fixed-price per size tier) — validated end-to-end. Azure adapter now bounds tiered meters (each tier ends at the next tier's start, not Inf)
- [x] GCP computed engine + `google_compute_instance` (Σ vCPU-core + GiB-RAM, formulaic catalog, zone→region); `google_compute_disk` (pd-standard/balanced/ssd, per-GiB), `google_compute_address` (static IP hourly), `google_storage_bucket` (Standard regional, band), `google_container_cluster` (GKE $0.10/hr mgmt fee) — validated end-to-end. GCP adapter skips leading $0 tiers; region resolution is case-insensitive (a bucket's uppercase `location`)

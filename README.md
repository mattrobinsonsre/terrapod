...
| Run tasks | Webhook hooks for external validation at three boundaries — `pre_plan`, `post_plan` and `pre_apply`. A mandatory `post_plan` failure can be overridden by an admin; `pre_plan` and `pre_apply` verdicts are final |
...
- **Run tasks** — external webhook validation hooks at `pre_plan`, `post_plan` and `pre_apply`. Overridability is chosen by boundary: a failed mandatory `post_plan` stage can be waived by an admin, while `pre_plan` and `pre_apply` are final (fix the cause and plan again). See [docs/run-tasks.md](docs/run-tasks.md).

...
| `policy-path` | string | Directory within the repo containing the set's files. Only direct children are loaded (no recursive descent). Empty string means repo root. Since v1.8.0 this takes `.rego` **and** `.yaml`/`.yml`/`.json` data files, plus `.rego` helpers defining no `deny`/`warn` — so any YAML or JSON in this directory is carried into the set. `*_test.rego` is never synced, and a file over 1 MiB is skipped and reported on `vcs-last-error`. |
| `shared-evaluation` | boolean | Evaluate the set's files together in one `opa eval`, so policies can call shared helper rules and read the data files committed beside them. Defaults to `false` — every set evaluated one policy at a time before this existed. Results become per-SET rather than per-policy: the files share `package terrapod`, so OPA cannot attribute a denial to one file. **Requires runners new enough to understand it** — see [`policies.md`](policies.md#runner-version), because an older runner evaluates without the data files and a rule reading `data.x` then passes rather than failing. |
| `support-file-names` | array | Read-only. The data files and helpers the last sync found, sorted. Use it to confirm your files were picked up; an empty list on a set with `shared-evaluation` on usually means the wrong `policy-path`. Written only by the VCS sync — there is no way to set it through the API. |

...
60 seconds, never by contacting Vault on the request. A collection of
...
{"data": [{"id": "default", "type": "vault-instance-statuses", "attributes": {
   ...
   "last-error": {"class": "VaultDenied", "at": "2026-09-15T09:12:03Z",
     "message": "variable 'NETBOX_TOKEN': Vault denied 'secret/apps/netbox' on instance 'default'. …"}}}],
 "meta": {"pagination": {…},
...
a sealed Vault is never logged in to), which is not the same as `false`. With
Vault off, `data` is empty and `meta.vault.enabled` is `false`.
...
`pass`, `fail`, `skipped` or `unknown`, where `unknown` means Vault could not

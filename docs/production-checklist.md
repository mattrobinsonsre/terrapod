...
- [ ] **No role sets `allow-all` unintentionally** — it grants on every resource on every axis, including ones created later. Verify each role's real reach with `GET /api/v1/roles/{name}/preview`, which reports it across all axes and names the rule responsible for each match.

...
```
POST /api/v1/workspaces/{id}/run-tasks
```
...
```
GET /api/v1/workspaces/{id}/run-tasks
```

#### Show Task

```
GET /api/v1/run-tasks/{id}
```

#### Update Task

```
PATCH /api/v1/run-tasks/{id}
```

#### Delete Task

```
DELETE /api/v1/run-tasks/{id}
```
...
```
PATCH /api/v1/task-stage-results/{id}/callback
```
...
```json
{
  ...
  "task_result_callback_url": "https://terrapod.local/api/v1/task-stage-results/tsr-id/callback",
  "run_id": "run-uuid",
  "run_status": "planning",
...
```bash
curl -X POST https://terrapod.local/api/v1/workspaces/ws-ID/run-tasks \
  -H "Authorization: Bearer $TOKEN" \

...
| **Workspaces** | Name, execution mode, terraform/tofu version, working directory, auto-apply (written as a **mode** — TFE's boolean maps `true` → `always`, `false` → `never`; the conditional `create` / `create_update` modes have no TFE equivalent, so choose them after migrating), owner, VCS repo + branch link. TFE **tags → Terrapod labels** (`"k:v"` → `{k: "v"}`, bare `"k"` → `{k: ""}`). |

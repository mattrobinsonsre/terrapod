...
| [Vault](vault.md) | HashiCorp Vault as a variable value source: a variable holds a reference resolved at run time, so Vault stays the source of truth and nothing is stored in Terrapod. Static kv-v2 and dynamic engines (a fresh credential per run); Kubernetes auth by default, JWT auth for a Vault outside the cluster (incl. HCP Vault Dedicated), AppRole and token; multiple Vaults; a private CA and a path allow-list per instance; an admin status page per instance, and a reference **Check** that never mints a credential, for diagnosing a variable that won't resolve |

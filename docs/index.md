...
| **Module Autodiscovery** | [Rules](registry.md#module-autodiscovery) that find the modules in a repository (root and submodules): preview, register all or a picked subset, and register new directories automatically; never deletes a module |
...
| [Terraform provider](terraform-provider.md) | `terraform-provider-terrapod` — manage Terrapod itself as code (workspaces, variables, roles, VCS connections, agent pools, run tasks, catalog, execution hooks, …) via 26 `terrapod_*` resources + 10 data sources; served per-instance and GPG-signed |
...
| [Package Proxies](package-cache.md) | Pull-through PyPI and npm so a run resolves its language dependencies with no upstream reach — `PIP_INDEX_URL` / `NPM_CONFIG_REGISTRY`. Upstream integrity metadata passed through untouched; a sealed node serves an index of what it actually holds |
| [Container Registry](oci-registry.md) | An OCI registry at `/v2/` — push and pull images with docker/skopeo/crane or a Kubernetes `imagePullSecret`, pull-through mirroring of upstream registries behind an allow-list, content-addressed shared storage, and the referrers API for signatures/SBOMs. Authenticated for reads as well as writes |

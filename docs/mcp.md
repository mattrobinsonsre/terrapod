...
| `terrapod_policy_set_list` | The OPA policy sets and how each is scoped — enforcement level, whether it is enabled, its allow/deny rules, and any VCS sync error. What answers "why was this run blocked" and "what governs this workspace". Platform admin only; read-only, because changing a gate that can hold every apply belongs in a plan a person reads. |
| `terrapod_run_policy_checks` | A run's policy checks as the `tofu`/`terraform` CLI sees them: one for its OPA policy sets and one for its security scan, each with status (`passed`, `soft_failed`, `overridden`), whether it can be overridden and whether the caller may, and its output. |
...
| `terrapod_run_ai_policy` | A run's AI policy gate verdict (#1766): the model's allow/deny ruling against the operator's natural-language deny criteria, the reasons it matched (criterion + Terraform address), the summary's risk level, the enforcement level, the outcome and any override. Note `errored` BLOCKS under a mandatory gate rather than passing — the gate fails closed, and `error` distinguishes a spent token budget from a model fault. Null when no verdict is recorded; the meta then says whether the gate is off, the engine is not ruled on (Pulumi), or the verdict has simply not landed yet. |
...
| `terrapod_module_autodiscovery_rule_scan` | — | Register the modules a module autodiscovery rule finds — every candidate, or a subset you pass (preview first): `subdirectories` for a single-repository rule, `selections` (repositories, each with optional subdirectories) for a rule over an org or group. Skips candidates already registered or whose name is taken, and reports them. Creates registry modules; touches no infrastructure. Platform admin only. |
| `terrapod_policy_check_override` | gated | Override a soft-failed policy check (from `terrapod_run_policy_checks`) so the run it holds can proceed: the OPA check overrides every failed policy set, the scan check overrides the scan. Workspace admin only. This bypasses a governance gate — read the output first. |
...
| `terrapod_run_ai_policy_override` | gated | Release a run held by the AI policy gate so it can proceed despite a deny or an unreachable verdict; a run still held in planning is re-driven at once. Workspace admin only. This bypasses a governance gate — read the verdict's reasons first and prefer fixing what they name. Also releases a run held with NO verdict recorded — the summariser never ran, or failed before ruling — which is the case a mandatory gate cannot otherwise escape; that records an explicit no-verdict override rather than a pass. |
...
| `terrapod_registry_module_interface` | A module version's **inputs + outputs** — the exact surface to author a correct `module` block against it, instead of guessing variable names. `interface-error` is present when the module could not be parsed, so empty inputs are not "no variables". |
| `terrapod_module_autodiscovery_rule_list` | The module autodiscovery rules — each names a repository, an org or group, or a repository-name pattern (`target-kind`), which directories count as modules (a glob pattern and ignore paths) and how they are named, and any `last-error`. Platform admin only. |
| `terrapod_module_autodiscovery_rule_preview` | What a rule finds now: each module directory (root and submodules) with its repository, the name and provider it would get, the module already registered from it, and whether its name is taken. A rule over an org or group is served from what the last poll found, a page of repositories at a time (`page_number`, `page_size`), or reads one `repository` live. Registers nothing. Platform admin only. |
| `terrapod_module_autodiscovery_rule_repositories` | The repositories a rule looks at, each with its status (active, archived, empty, no-branch, out-of-scope, covered, error), origin (baseline or new), candidates, last skips, previous paths and last error; filter by `status`, paged (default 50). Use it to see why a repository registered nothing. Platform admin only. |

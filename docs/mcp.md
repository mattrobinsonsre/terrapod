...
| `terrapod_run_policy_checks` | A run's OPA policy set results: which sets were evaluated, how each ruled, the violations they produced, and which one is holding the run. `blocking` is true only when a **mandatory** set failed or errored and was not overridden — an advisory failure is recorded and never blocks, so the outcome alone does not say whether the run is stuck. This is what answers "why is this run blocked" when `blocked-by` is `policy`; pair it with `terrapod_policy_set_list` to see what the named set is and who else it applies to. |
